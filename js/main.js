...
function validation(e) {
  ...
  const userMonth = Math.floor(
    (diferenciaEnMilisegundos % milisegundosEnUnAño) /
      (1000 * 60 * 60 * 24 * 30.44)
  );
  const userDay = Math.floor(
...
function validationDay(valuex, valuey, valuez, valuek, valuem) {
  const dayMonth = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  if (valuex >= 1 && valuex <= dayMonth[valuem - 1]) {
